...
Run from the command line::
    ...
    uv run dyco-apply-batch --help

or via the module form::

    uv run python -m dyco.apply_tlag --help

**Required flags**

``--input-dir PATH``
    Directory containing the original EddyPro rotated ``.txt`` files (the same
    directory that was passed to ``dyco-pwb-batch``).
``--output-dir PATH``
    Output directory for the lag-corrected files.
``--results-csv PATH``
    The ``tlag_results.csv`` produced by ``dyco-pwb-batch``.
``--scalar LABEL:column``
...
from dyco.pwb import _DEFAULT_NA_VALUES, _read_engine_kwargs
...
class TlagApplier:
    ...
    input_dir : Path
        ...
        same directory passed to ``dyco-pwb-batch``).
    output_dir : Path
...
def _build_parser():
    ...
    p = argparse.ArgumentParser(
        prog='python -m dyco.apply_tlag',
        description=(
            'Apply PWB-detected time lags to raw EC files.\n'
            'Alias: uv run dyco-apply-batch'
        ),
    ...
    p.add_argument('--results-csv', required=True,
                   help='tlag_results.csv produced by dyco-pwb-batch.')
