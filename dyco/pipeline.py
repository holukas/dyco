...
2. Apply double rotation to the wind vector (u, v, w) of the chunk
   **in memory** using ``dyco.rotation.WindDoubleRotation``. The rotated W
...
4. Shift each scalar column in the **unrotated** chunk backward by
   ...
   ``dyco-apply-batch`` removes). A wide-HDI chunk's *raw* mode lag
...
::
    ...
    uv run dyco-detect-remove --help
    uv run python -m dyco.pipeline --help

Example (raw CSV: header on line 1, 2 extra rows for units + source)::

    uv run dyco-detect-remove ^
...
- ``<--detect-subdir>/detect_and_remove_tlag_summary.csv`` (default
  ...
  mirrors ``tlag_results.csv`` from ``dyco-pwb-batch`` plus this
...
# Suppress the runpy double-import warning that fires when ``dyco`` package init
...
from dyco.pwb import (  # noqa: E402
    _DEFAULT_NA_VALUES,
    _read_engine_kwargs,
...
from dyco._vendor.console import warn  # noqa: E402
...
def _write_raw_file(
    ...
    with _open_text_write(output_path) as fh:
...
def _ensure_headless_backend() -> None:
    ...
    of each plotting path rather than at module import — keeps ``import dyco``
        ...
        for ci in range(n_chunks):
            ...
            try:
                ...
                for gi, (label, col_name) in enumerate(scalars.items()):
                    ...
                    # Full schema (matches dyco-pwb-batch results CSV)
...
def _empty_detect_row(scalars: dict, period: str, parent: str,
    ...
    Schema mirrors ``tlag_results.csv`` from ``dyco-pwb-batch`` so the
...
def _estimate_data_rows(path: Path, header_lines: int,
    ...
    if _is_compressed(path):
        # stat() reports the *compressed* size, but the sample below is read
        # decompressed - scaling one by the other mixes units and underestimates
        # the row count by roughly the compression ratio (~5x on raw EC data),
        # which silently drops most of a file's chunks. Counting exactly means
        # decompressing, which this function would have to do anyway, so there
        # is no saving to protect here.
...
def _summary_columns_doc(scalars: dict, lag_column_template: str,
    ...
    lines.append(
        ...
        "pipeline (`dyco-detect-remove`). Each raw input file is "
...
class PerFilePipeline:
    ...
    def _write_run_settings(self) -> None:
        ...
        lines = [
            'dyco PWB time-lag detect + remove -- run settings',
            '==================================================',
            '',
            ...
            f'produced by dyco-detect-remove into: {self.output_dir}',
            '',
        ]
        lines += _block('paths', [
    ...
    def _write_readme(self) -> None:
        ...
        text = (
            "dyco PWB time-lag detection + removal -- output folder\n"
            "=======================================================\n"
            "\n"
            "Produced by dyco-detect-remove (PerFilePipeline): a\n"
    ...
    def _apply_pwbopt_postprocessing(self, summary: DataFrame) -> DataFrame:
        ...
        Matches the schema produced by ``dyco-pwb-batch``'s
...
def _build_parser():
    ...
    p.add_argument('--lag-column-template', default='{prefix}_tlag_final_pf_s',
                   help='Which PWBOPT lag column to actually remove in phase '
                        ...
                        'gap-filled "best" lag; matches dyco-apply-batch). '
