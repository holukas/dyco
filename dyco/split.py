"""
SPLIT: DIVIDE LARGE RAW EC FILES INTO SHORTER PARTS
====================================================

Split one long raw eddy covariance file into time-based parts, optionally
rotating the wind and writing turbulent departures alongside.

Moved here from diive (`core/io/filesplitter.py`), where it was unreferenced.
Splitting a long file into averaging-period chunks is the same operation the lag
pipeline performs before detection, so it belongs with the lag code.

[PORT NOTE] diive's version read files through `ReadFileType`, its YAML-driven
filetype reader. That is replaced by dyco's own `files.read_raw_data`, so the
constructor takes `data_timestamp_format` instead of `filetype`. The file is
read without parsing a timestamp - the index is rebuilt from the record count by
`create_timestamp` on the very next line, which is what the original did too.
`save_parquet` is likewise replaced by a plain `DataFrame.to_parquet`; diive's
version added a format enforcement that raw high-frequency data does not want.
...
from dyco._vendor import filedetector as fd
from dyco._vendor.console import info, warn
from dyco._vendor.fileio import search_files
from dyco._vendor.frames import trim_frame
from dyco._vendor.times import create_timestamp
...
class FileSplitter:
    ...
    def __init__(
            ...
            file_start: str,
            outdir: str,
            data_timestamp_format: str = None,
        ...
        Args:
            ...
            data_timestamp_format: Datetime format of a timestamp column in the raw file, or None
                (default) when the file has no usable timestamp. None is the normal case here:
                the index is rebuilt from the record count by `create_timestamp`.
    ...
    def run(self):
        ...
        # Read file
        file_df = read_raw_data(filepath=self.filepath,
    ...
    def _loop_splits(self, file_df, outdir_splits):
        ...
        for split_key, split_df in split_grouped:
            ...
            # Save as parquet
                ...
                split_filepath = Path(outdir_splits) / f"{split_name}.parquet"
                split_df.to_parquet(split_filepath)
                split_name_ext = split_filepath.name
