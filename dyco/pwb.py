...
**Example — PowerShell / Windows (one-liner)**::
    ...
    uv run dyco-pwb-batch --input-dir "F:\\path\\to\\input" --output-dir "F:\\path\\to\\output" --scalar CH4:ch4 --scalar N2O:n2o --col-w w --col-tsonic ts --usecols 0 1 2 3 6 7 --col-names u v w ts ch4 n2o --skiprows 9 --hz 20 --lag-max 10.0 --n-bootstrap 99 --block-length 20.0 --min-valid-frac 0.3 --hdi-thresh 0.5 --dev-thresh 0.5 --hdi-prefilter 1.0 --n-workers 16 --save-plots
...
# when the package init has already imported this module before -m re-executes it.
...
from dyco._vendor import plotstyle as theme
from dyco._vendor.console import warn
...
class PwboptLagPlot:
    ...
    Follows the two-phase plotting pattern:
...
def _parse_file_timestamp(
    ...
    *file_date_format* is a ``datetime.strptime`` pattern for the datetime token
    (``file_date_format`` convention), e.g. ``'%Y%m%d-%H%M'`` for
...
class PwbBatchDetection:
    ...
    file_date_format:
        ...
        each filename (``file_date_format`` convention), e.g.
