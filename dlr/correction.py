...
class NormalizeLags:
    ...
    def run(self):
        ...
        for file_idx, file_info_row in self.files_overview_df.iterrows():
            ...
            data_df = self.remove_default_lag(df=data_df,
                                              shift=int(self.lut_default_lag_times_df.loc[this_date]['correction']))

            self.save_dlr(outdir=self.outdirs['3-1_____Normalized_Files'],
            ...
            txt_info = f"File #{files_counter}: {file_info_row['filename']}" \
                       f"    time needed: {time_needed:.3f}s    remaining time: {remaining_sec:.0f}s" \
                       f"    remaining files: {remaining_files}    progress: {progress:.2f}%"
            print(txt_info)

    def save_dlr(self, df, outdir, original_filename, export_timestamp):
        df.fillna(-9999, inplace=True)
        outpath = outdir / f"{original_filename}_DLR.csv"
        df.to_csv(outpath, index=export_timestamp)

    def remove_default_lag(self, df, shift):
    ...
    def read_lut(self):
        filepath = self.outdirs['3-0_DLR_Lookup_Table_Normalization'] / f'LUT_default_lag_times.csv'
