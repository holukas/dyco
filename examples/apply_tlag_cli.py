...
Demonstrates calling ``TlagApplier`` from the command line via::
    ...
    python -m dyco.apply_tlag [options]

or the installed console-script alias::

    uv run dyco-apply-batch [options]

This script:

1. Generates synthetic EddyPro-format raw files in a temporary input
   directory.
2. Runs ``dyco-pwb-batch`` to detect time lags and write a
   ``tlag_results.csv``.
3. Runs ``dyco-apply-batch`` to read that results CSV and shift the
...
.. code-block:: text
    ...
    uv run dyco-apply-batch --input-dir "F:\\path\\to\\raw_files" --output-dir "F:\\path\\to\\aligned_files" --results-csv "F:\\path\\to\\01_pwb_results\\tlag_results.csv" --scalar CH4:ch4 --scalar N2O:n2o --hz 20 --skiprows 9 --n-workers 16
...
if __name__ == '__main__':
    ...
    apply_cmd = [
        sys.executable, '-m', 'dyco.apply_tlag',
        '--input-dir', input_dir,
