...
Short alias (after ``uv sync`` or ``pip install dyco``)
---------------------------------------------------------
.. code-block:: text

    uv run dyco-pwb-batch --input-dir ... --output-dir ... [options]
    uv run dyco-pwb-batch --help

``dyco-pwb-batch`` is a console-script entry point defined in
``pyproject.toml`` that calls the same ``_cli_main()`` function as
``python -m dyco.pwb``.

Run ``uv run dyco-pwb-batch --help`` for all options.
"""
...
if __name__ == '__main__':
    ...
    # Run the CLI
    # ------------------------------------------------------------------
    # Suppress the runpy double-import warning that appears when the package init
