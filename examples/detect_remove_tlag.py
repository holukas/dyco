...
``PerFilePipeline`` (CLI: ``dyco-detect-remove``) reads each long
...
Run ``uv run dyco-detect-remove --help`` for every flag, or drive
the same pipeline from a terminal UI with
``uv run dyco-detect-remove-tui`` (``--demo`` previews the interface
without any data).
...
if __name__ == '__main__':
    ...
    # tlag_results.csv from dyco-pwb-batch.
